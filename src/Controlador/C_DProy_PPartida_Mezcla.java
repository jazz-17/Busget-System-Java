...
public class C_DProy_PPartida_Mezcla implements ActionListener, MouseListener, ItemListener {
    ...
    public void initListarProyectos_I() {
        ...
        for (int i = 0; i < listaPI.size(); i++) {
            ...
            Proyecto pyto = listaPI.get(i);
            SelectOption obj = new SelectOption(pyto.getNomPyto() + "", pyto.getCodPyto() + "");
            vpm.codPyto_I.addItem(obj);
        }
    }
    ...
    public void initListarProyectos_E() {
        ...
        for (int i = 0; i < listaPE.size(); i++) {
            Proyecto pyto = listaPE.get(i);
            SelectOption obj = new SelectOption(pyto.getNomPyto() + "", pyto.getCodPyto() + "");
            vpm.codPyto_E.addItem(obj);
        }
    }
    ...
    public void initListarPartidas(int codpyto) {
        ...
        for (int i = 0; i < listaParI.size(); i++) {
            SelectOption obj = new SelectOption(listaParI.get(i).getDescription(),
                    listaParI.get(i).getCodPartida() + "");
            vpm.codPartida_I.addItem(obj);
        }

        for (int i = 0; i < listaParE.size(); i++) {
            SelectOption obj = new SelectOption(listaParE.get(i).getDescription(),
                    listaParE.get(i).getCodPartida() + "");
            vpm.codPartida_E.addItem(obj);
        }
    }
    ...
    public void mouseClicked(MouseEvent e) {
        ...
        if (e.getSource() == vpm.tablaDProy_Partida_I) {
            ...
            vpm.codPartida_I.setSelectedItem(obj);
            
            int codPyto = pmI.getCodPyto();
            ...
            vpm.codPyto_I.setSelectedItem(obj2);
    ...
    public void registrarDatos(String tip) {
        ...
        if (tip == "I") {
            ...
            try {
                pm.setCodCia(varCodCiaGlobalDeLogin);
                Object item = vpm.codPyto_E.getSelectedItem();
                String value = ((SelectOption) item).getValue();
                pm.setCodPyto(Integer.parseInt(value));
                pm.setIngEgr(tip);
    ...
    public void eliminarDatos(String tip) {
        ...
        if (tip == "I") {
            fila = vpm.tablaDProy_Partida_I.getSelectedRow();
            sec = Integer.parseInt(vpm.tablaDProy_Partida_I.getValueAt(fila, 4).toString());

        } else {
            fila = vpm.tablaDProy_Partida_E.getSelectedRow();
            sec = Integer.parseInt(vpm.tablaDProy_Partida_E.getValueAt(fila, 4).toString());
        }
        dpmDAO.eliminar(sec);
        // complete it tho
    }
    ...
    public void itemStateChanged(ItemEvent e) {
        if (e.getSource() == vpm.codPyto_I) {
            if (vpm.codPyto_I.getSelectedIndex() != -1) {
                Object item = vpm.codPyto_I.getSelectedItem();
                String value = ((SelectOption) item).getValue();
                initListarPartidas(Integer.parseInt(value));
            } else {
