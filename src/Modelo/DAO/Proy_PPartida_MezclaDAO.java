...
public class Proy_PPartida_MezclaDAO implements CRUD<Proy_PPartida_Mezcla> {
    ...
    @Override
    public List<Proy_PPartida_Mezcla> listar() {
        List<Proy_PPartida_Mezcla> lista = new ArrayList<>();
        String sql = "SELECT * FROM PROY_PPARTIDA_MEZCLA order by CODCIA";
        try {
            con = conexion.conectar();
            ps = con.prepareStatement(sql);
            rs = ps.executeQuery(sql);
            while (rs.next()) {
                Proy_PPartida_Mezcla ppm = new Proy_PPartida_Mezcla();
    ...
    @Override
    public int add(Proy_PPartida_Mezcla p) {
        System.out.println("{call INSERTAR_PROY_PPARTIDA_MEZCLA(?,?,?,?,?,?)}");
        try {
            con = conexion.conectar();
            myCall = con.prepareCall("{call INSERTAR_PROY_PPARTIDA_MEZCLA(?,?,?,?,?,?)}");
    ...
    @Override
    public int actualizar(Proy_PPartida_Mezcla p) {
        String sql1 = "update PROY_PPARTIDA_MEZCLA set NROVERSION=?,NIVEL=?,ORDEN=?,COSTOUNIT=?,CANT=?,COSTOTOT=? where CODCIA=? AND CODPYTO=? AND INGEGR=? AND CORR=? AND CODPARTIDA=?";
    ...
    public void eliminar(int id) {
        String sql1 = "DELETE from PROY_PPARTIDA_MEZCLA where CORR=" + id;
        try {
            con = conexion.conectar();
            ...
            ps.close();
            con.close();
            JOptionPane.showMessageDialog(null, "Proy_PPartida_Mezcla eliminado con exito.");
        } catch (Exception e) {
    ...
    public List<Proy_PPartida_Mezcla> listarPorCodCiaYProyecto(int id, int pyto, String tip) {
        List<Proy_PPartida_Mezcla> lista = new ArrayList<>();
        String sql = "SELECT "
                ...
                + "FROM PROY_PPARTIDA_MEZCLA ppm "
                + "LEFT JOIN proy_ppartida pp ON ppm.codpartida = pp.codpartida AND ppm.nroversion = pp.nroversion "
                ...
                + "LEFT JOIN proy_ppartida pp2 ON ppm.padcodpartida = pp2.codpartida AND ppm.nroversion = pp2.nroversion "
                + "AND ppm.ingegr = pp2.ingegr AND ppm.codcia = pp2.codcia AND ppm.codpyto = pp2.codpyto "
                + "LEFT JOIN ppartida p2 ON pp2.codpartida = p2.codpartida AND pp2.codcia = p2.codcia AND pp2.ingegr = p2.ingegr "
    ...
    public List<Proy_PPartida_Mezcla> listarPorCodCia(int id, String tip) {
        List<Proy_PPartida_Mezcla> lista = new ArrayList<>();
        String sql = "SELECT "
                + "codpyto,nroversion,CORR,codpartida,padcodpartida,nivel,orden,tunimed,eunimed,costounit,cant,costotot "
                + "FROM PROY_PPARTIDA_MEZCLA WHERE codcia=" + id + " AND ingegr='" + tip + "' order by CORR";
        System.out.println(sql);
        try {
            con = conexion.conectar();
            ps = con.prepareStatement(sql);
    ...
    public List<Proy_PPartida_Mezcla> listarCodPartida_Mezcla(int id, String tip, int pyto) {
        List<Proy_PPartida_Mezcla> lista = new ArrayList<>();
        String sql = "SELECT "
                + "codpartida"
                + " FROM PROY_PPARTIDA_MEZCLA WHERE codcia=" + id + " AND ingegr='" + tip + "' AND CODPYTO=" + pyto
                + " order by codpartida";
        System.out.println(sql);
        try {
            con = conexion.conectar();
            ...
            rs = ps.executeQuery(sql);
            while (rs.next()) {
                Proy_PPartida_Mezcla pm = new Proy_PPartida_Mezcla();
    ...
    public List<Proy_PPartida_Mezcla> listarNiveles(int codC, int codProy, String tipo) {
        List<Proy_PPartida_Mezcla> lista = new ArrayList<>();
        String sql = "SELECT CODPARTIDA, NIVEL FROM PROY_PPARTIDA_MEZCLA WHERE codcia=" + codC + " AND codpyto="
    ...
    public Proy_PPartida_Mezcla listarId(int cia, String tipo, int id, int corr, int ver, int pyto) {
        Proy_PPartida_Mezcla pm = new Proy_PPartida_Mezcla();
        String sql = "SELECT CODPYTO,NROVERSION,CODPARTIDA,PADCODPARTIDA,NIVEL,ORDEN,COSTOUNIT,"
                + "CANT FROM PROY_PPARTIDA_MEZCLA WHERE CODCIA=" + cia + " AND INGEGR='" + tipo + "' "
    ...
    public Proy_PPartida_Mezcla listarId2(int cia, String tipo, int id, int pyto) {
        Proy_PPartida_Mezcla pm = new Proy_PPartida_Mezcla();
        String sql = "SELECT NROVERSION,CORR,ORDEN"
                + " FROM PROY_PPARTIDA_MEZCLA WHERE CODCIA=" + cia + " AND INGEGR='" + tipo + "' "
    ...
    public void eliminarDatos(int cia, int cod, String tipo, int corr, int pyto, int ver) {
        String sql1 = "DELETE from PROY_PPARTIDA_MEZCLA where CODCIA=" + cia + " AND"
    ...
    public int asignarOrden(String tip, int nivel, int cia, int pyto) {
        int orden = 0;
        String sql = "SELECT * FROM (SELECT ORDEN FROM PROY_PPARTIDA_MEZCLA WHERE INGEGR='" + tip + "' AND NIVEL="
    ...
    public int asignarOrdenNoNULL(String tip, int cod, int cia, int pyto) {
        int orden = 0;
        String sql = "SELECT * FROM (SELECT ORDEN FROM PROY_PPARTIDA_MEZCLA WHERE INGEGR='" + tip
    ...
    public int asignarNivelNoNULL(String tip, int cod, int cia, int pyto) {
        int nivel = 0;
        String sql = "SELECT * FROM (SELECT NIVEL FROM PROY_PPARTIDA_MEZCLA WHERE INGEGR='" + tip + "' AND CODPARTIDA="
    ...
    public List<Proy_PPartida_Mezcla> listarCodPartida(int id, String tip, int pyto) {
        List<Proy_PPartida_Mezcla> lista = new ArrayList<>();
        String sql = "SELECT "
                + "ppm.codpartida, p.despartida"
                + " FROM PROY_PPARTIDA_MEZCLA ppm" 
                + " LEFT JOIN proy_ppartida pp on pp.codpartida = ppm.codpartida AND pp.ingegr = ppm.ingegr AND pp.nroversion = ppm.nroversion AND pp.codpyto = ppm.codpyto AND pp.codcia = ppm.codcia"
                + " LEFT JOIN ppartida p on p.codpartida = pp.codpartida AND p.ingegr = pp.ingegr AND p.codcia = pp.codcia"
                + " WHERE ppm.codcia=" + id + " AND ppm.ingegr='" + tip + "' AND ppm.CODPYTO=" + pyto
    ...
    public float buscarTotalCodPartCodProy(String pyto, String codpart) {
        float costo = 0;
        String sql = "SELECT "
                + "COSTOTOT FROM PROY_PPARTIDA_MEZCLA WHERE CODPYTO=" + pyto + " AND CODPARTIDA=" + codpart;
        System.out.println(sql);
        try {
            con = conexion.conectar();
            ps = con.prepareStatement(sql);
    ...
    public List<Proy_PPartida_Mezcla> eliminarCodPartida(List<Proy_PPartida_Mezcla> lista, int id, String tip, int pyto) {

        String sql = "SELECT DISTINCT PADCODPARTIDA FROM PROY_PPARTIDA_MEZCLA WHERE CODCIA=" + id + " AND "
