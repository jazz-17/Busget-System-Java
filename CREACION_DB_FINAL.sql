...
create table persona (
	...
	vigente varchar2(1) not null,
	constraint cia_persona_pk primary key (
		codcia,
		codpersona
	)
);
...
create table empresa_vta (
	...
	vigente varchar2(1) not null,
	constraint empresa_vta_pk primary key (
		codcia,
		ciacontrata
	)
);
...
create table partida_mezcla (
	...
	vigente varchar2(1) not null,
	constraint partida_mezcla_pk primary key (
		codcia,
		ingegr,
		codpartida,
		corr
...
create table proy_partida_mezcla (
	...
	constraint proy_partida_mezcla_pk primary key (
		codcia,
		codpyto,
		nroversion,
		ingegr,
...
create table proy_partida (
	...
	codestado varchar2(3) not null,
	vigente varchar2(1) not null,
	constraint proy_partida_pk primary key (
		codcia,
		codpyto,
		nroversion,
		ingegr,
...
create table elementos (
	...
	vigente varchar2(1) not null,
	constraint elementos_pk primary key (
		codtab,
		codelem
	)
);
...
create table dproy_partida_mezcla (
	...
	semilla number(5) not null,
	constraint dproy_partida_mezcla_pk primary key (
		codcia,
		codpyto,
		ingegr,
		nroversion,
...
create table comp_pagocab (
	...
	tabestado varchar2(3) not null,
	codestado varchar2(3) not null,
	constraint comp_pagocab_pk primary key (
		codcia,
		codproveedor,
		nrocp
	)
...
create table comp_pagodet (
	...
	semilla number(5) not null,
	constraint comp_pagodet_pk primary key (
		codcia,
		codproveedor,
		nrocp,
		sec
...
create table vtacomp_pagocab (
	...
	tabestado varchar2(3) not null,
	codestado varchar2(3) not null,
	constraint vtacomp_pagocab_pk primary key (
		codcia,
		nrocp
	)
);
create table vtacomp_pagodet (
	...
	imptotalmn number(9, 2) not null,
	semilla number(5) not null,
	constraint vtacomp_pagodet_pk primary key (
		codcia,
		nrocp,
		sec
	)
...
create table flujocaja (
	...
	codestado varchar2(3) not null,
	vigente varchar2(1) not null,
	constraint flujocaja_pk primary key (
		codcia,
		codpyto,
		ingegr,
		tipo,
...
create table flujocaja_det (
	...
	constraint flujocaja_det_pk primary key (
		anno,
		codcia,
		codpyto,
		ingegr,
...
/*==============================================================*/
alter table persona
add constraint persona_empresa_vta_fk foreign key (codcia) references cia (codcia);
/*==============================================================*/
/* PROVEEDOR                                                    */
/*==============================================================*/
...
/*==============================================================*/
alter table cliente
add constraint persona_cliente_fk foreign key (
		codcia,
		codcliente
	) references persona (codcia, codpersona);
/*==============================================================*/
...
add constraint persona_empresa_vta_cia_fk foreign key (
		codcia,
		ciacontrata
	) references persona (codcia, codpersona);
/*==============================================================*/
...
add constraint persona_empleado_fk foreign key (
		codcia,
		codempleado
	) references persona (codcia, codpersona);
/*==============================================================*/
...
add constraint cia_proyecto_fk foreign key (codcia) references cia (codcia);
alter table proyecto
add constraint empleado_proyecto_fk foreign key (
		codcia,
		empljefeproy
	) references empleado (codcia, codempleado);
alter table proyecto
add constraint cliente_proyecto_fk foreign key (
		codcia,
		codcliente
	) references cliente (codcia, codcliente);
alter table proyecto
add constraint empresa_vta_proyecto_fk foreign key (
		codcia,
		ciacontrata
...
/*==============================================================*/
alter table partida
add constraint cia_partidafk foreign key (codcia) references cia (codcia);
/*==============================================================*/
/* PROY_PARTIDA                                                 */
/*==============================================================*/
alter table proy_partida
add constraint proyecto_proy_partida_fk foreign key (
		codcia,
		codpyto
	) references proyecto (codcia, codpyto);
alter table proy_partida
add constraint partida_proy_partida_fk foreign key (
		codcia,
		ingegr,
		codpartida
	) references partida (
...
add constraint partida_partida_mezcla_fk foreign key (
		codcia,
		ingegr,
		codpartida
	) references partida (
...
alter table partida_mezcla
add constraint elementos_partida_mezcla_fk foreign key (
		tunimed,
		eunimed
...
add constraint proy_partida_proy_partida_mezcla_fk foreign key (
		codcia,
		codpyto,
		nroversion,
		ingegr,
		codpartida
	) references proy_partida (
		codcia,
		codpyto,
		nroversion,
		ingegr,
...
alter table proy_partida_mezcla
add constraint elementos_proy_partida_mezcla_fk foreign key (
		tunimed,
		eunimed
	) references elementos (codtab, codelem);
/*==============================================================*/
...
add constraint proy_partida_mezcla_dproy_partida_mezcla_fk foreign key (
		codcia,
		codpyto,
		ingegr,
		nroversion,
		codpartida,
		corr
	) references proy_partida_mezcla (
		codcia,
		codpyto,
		ingegr,
		nroversion,
...
alter table dproy_partida_mezcla
add constraint elementos_dproy_partida_mezcla_desembolso_fk foreign key (
		tdesembolso,
		edesembolso
	) references elementos (codtab, codelem);
alter table dproy_partida_mezcla
add constraint elementos_dproy_partida_mezcla_comprobante_fk foreign key (
		tcomppago,
		ecomppago
	) references elementos (codtab, codelem);
/*==============================================================*/
...
add constraint elementos_tabs_fk foreign key (codtab) references tabs (codtab);
/*==============================================================*/
/* TABS                                                         */
/*==============================================================*/
/*NO TIENE*/
...
add constraint comp_pagocab_proveedor_fk foreign key (
		codcia,
		codproveedor
	) references proveedor (codcia, codproveedor);
alter table comp_pagocab
add constraint comp_pagocab_elementos_fk foreign key (
		tmoneda,
		emoneda
	) references elementos (codtab, codelem);
alter table comp_pagocab
add constraint comp_pagocab_elementos_2_fk foreign key (
		tcomppago,
		ecomppago
	) references elementos (codtab, codelem);
alter table comp_pagocab
add constraint comp_pagocab_proyecto_fk foreign key (
		codcia,
		codpyto
	) references proyecto (codcia, codpyto);
...
add constraint comp_pagodet_comp_pagocab_fk foreign key (
		codcia,
		codproveedor,
		nrocp
	) references comp_pagocab (
		codcia,
		codproveedor,
		nrocp
	);
alter table comp_pagodet
add constraint comp_pagodet_partida_fk foreign key (
		codcia,
		ingegr,
		codpartida
	) references partida (
...
add constraint vtacomp_pagocab_cliente_fk foreign key (
		codcia,
		codcliente
	) references cliente (codcia, codcliente);
alter table vtacomp_pagocab
add constraint vtacomp_pagocab_elementos_fk foreign key (
		tmoneda,
		emoneda
	) references elementos (codtab, codelem);
alter table vtacomp_pagocab
add constraint vtacomp_pagocab_elementos_2_fk foreign key (
		tcomppago,
		ecomppago
	) references elementos (codtab, codelem);
alter table vtacomp_pagocab
add constraint vtacomp_pagocab_proyecto_fk foreign key (
		codcia,
		codpyto
	) references proyecto (codcia, codpyto);
/* FALTA LLAVE FORANEA DE ESTADOS (TABLA INEXISTENTE)*/
...
add constraint vtacomp_pagodet_vtacomp_pagocab_fk foreign key (
		codcia,
		nrocp
	) references vtacomp_pagocab (codcia, nrocp);
alter table vtacomp_pagodet
add constraint vtacomp_pagodet_partida_fk foreign key (
		codcia,
		ingegr,
		codpartida
	) references partida (
...
add constraint partida_flujocaja_fk foreign key (
		codcia,
		ingegr,
		codpartida
	) references partida (
...
alter table flujocaja
add constraint proyecto_flujocaja_fk foreign key (
		codcia,
		codpyto
	) references proyecto (codcia, codpyto);
...
insert on proy_partida_mezcla for each row
declare annoauxiliar number(10);
...
order by n;
begin
select count(n) into numerodecolumnas
from (
		select distinct rownum n
		from dual connect by level <= (
				select annofin
...
open cursor_annos;
loop fetch cursor_annos into annoauxiliar;
...
create or replace function getpadre (
	...
	) return number is nropadre number;
begin
select padcodpartida into nropadre
...
/
create or replace function duracionproyecto (cod_pyto in proyecto.codpyto %type) return number is duracionenanios number;
begin
select count(n) into duracionenanios
from (
		select distinct rownum n
		from dual connect by level <= (
				select annofin
...
if proyectado_real = 'P' then for i in 1..12 loop column_name := case
	i
	when 1 then 'impene'
	when 2 then 'impfeb'
	when 3 then 'impmar'
...
if nrodemes = i then execute immediate 'select ' || column_name || ' from flujocaja_det where codcia = :1 and codpyto = :2 and ingegr = :3 and codpartida = :4 and anno = :5' into valorauxiliaractual using cod_cia,
cod_pyto,
ing_egr,
cod_partida,
nrodeanio;
execute immediate 'update flujocaja_det set ' || column_name || ' = :1 where codcia = :2 and codpyto = :3 and ingegr = :4 and codpartida = :5 and anno = :6' using valorauxiliaractual + costo_total,
cod_cia,
cod_pyto,
ing_egr,
cod_partida,
...
while getpadre(
	...
	ing_egr,
	cod_pyto
) <> 0 loop execute immediate 'select ' || column_name || ' from flujocaja_det where codcia = :1 and codpyto = :2 and ingegr = :3 and codpartida = :4 and anno = :5' into valorauxiliarpadre using cod_cia,
cod_pyto,
ing_egr,
getpadre(
	codhijo,
...
nrodeanio;
execute immediate 'update flujocaja_det set ' || column_name || ' = :1 where codcia = :2 and codpyto = :3 and ingegr = :4 and codpartida = :5 and anno = :6' using valorauxiliarpadre + costo_total,
cod_cia,
cod_pyto,
ing_egr,
getpadre(
...
set impacum = impini + impene + impfeb + impmar + impabr + impmay + impjun + impjul + impago + impsep + impoct + impnov + impdic
...
if proyectado_real = 'R' then for i in 1..12 loop column_name := case
	i
	when 1 then 'imprealene'
	when 2 then 'imprealfeb'
	when 3 then 'imprealmar'
	when 4 then 'imprealabr'
	when 5 then 'imprealmay'
	when 6 then 'imprealjun'
	when 7 then 'imprealjul'
	when 8 then 'imprealago'
	when 9 then 'imprealsep'
	when 10 then 'imprealoct'
	when 11 then 'imprealnov'
	when 12 then 'imprealdic'
end;
if nrodemes = i then execute immediate 'select ' || column_name || ' from flujocaja_det where codcia = :1 and codpyto = :2 and ingegr = :3 and codpartida = :4 and anno = :5' into valorauxiliaractual using cod_cia,
cod_pyto,
ing_egr,
...
while (contador > 0) loop contador := contador - 1;
update flujocaja_det f1
set f1.impini = nvl(
		(
			select f2.impacum
...
values (codc, sec_persona.currval, ruc, vig);
end;
/
/*INSERTAR CIA*/
...
insert into cliente
values (codcia, sec_persona.currval, ruc, vig);
end;
/
/*INSERTAR ELEMENTOS*/
...
values (ctab, cele, deele, decorta, vig);
end;
/
/*INSERTAR TABS*/
...
insert into dproy_partida_mezcla
values (
		codcia,
		codpyto,
		ingegr,
